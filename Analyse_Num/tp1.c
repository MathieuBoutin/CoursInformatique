...
matrix_t algoGradConjug(matrix_t A, matrix_t B, float epsi, int* errorCode, int *k,double* z)
{
	matrix_t X;
	...
	*z=sqrt(produitscalaireVecteur(r,r,errorCode))/b;
	//printf("z: %f \n",z);
...
int main()
{
	...
	X=algoGradConjug(A,B,epsi,&errorCode,&k,&z);
	printf("Méthode du gradient conjugué pour la matrice Pacal :\nLa solution X vaut:\n");
	printMatrix(X);
	printf("Le résidu vaut : %f\nLe nombre d'itération vaut: %d\n",z,k);
	...
	B=secondMembre(A,N,&errorCode);  
	X=algoGradConjug(A,B,epsi,&errorCode,&k,&z);
	printf("Méthode du gradient conjugué pour la matrice Hilbert :\nLa solution X vaut:\n");
	printMatrix(X);
	printf("Le résidu vaut : %f\nLe nombre d'itération vaut: %d\n",z,k);
